...
| 役割 | 採用 | 理由 |
|---|---|---|
| 天気・風・気温・波高・うねり・水温 | **Open-Meteo**(Standard $29/月・JMAモデル) | lat/lng・品質良好(PoC)・日本は JMA。forecast は対象日近くで再取得。CC BY 表記 |
| 潮の動き(上げ/下げ・満干) | **WWO 据え置き** | lat/lng で港コード不要・既存実装流用。潮の"動き"は天文計算でタイミング実用十分(消去法でもあるが妥当) |
| 水深・地形 | **OpenTopoData(GEBCO2020)** 本命 / **GMRT** 代替 | lat/lng で水深を綺麗な JSON・無料。GMRT は測深域で高解像度 |
...
> **不採用と理由:**
> - **海しる(MSIL)**: 潮汐推算は**リンク型 GeoJSON で数値を返さない**・カテゴリが粗く実用に難・地形も綺麗な点データAPIでない → 不採用(2026-05-29 実検証)。
> - **tide736.net**: 港コード指定で磯/河口を網羅しづらく lat/lng→港の変換工数 → 潮は WWO 据え置きを選択。
> - **NOAA NCEI**: REST が soundings 抽出主体で点深度に噛み合わず → OpenTopoData/GMRT を採用。
>
> 適用範囲: 天気/波の Open-Meteo 化と forecast 再取得は**既存 env data ごと移行**(ADR-0007)。潮は WWO のまま。水深は新規。

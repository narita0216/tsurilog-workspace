...
## 「釣れる度」算定ロジック(確定・2026-06-08)
当初の「各環境条件の釣れ率の単純平均」は、本番データのボウズ率が高く **全体が〜20%に潰れて偏る**問題があった(weather 48/206=23% が最大、等)。原因は魚種フィルタではなく(main は fish_id を送らず backend も絞っていない)、**ベース釣れ率が低い + ÷9固定 + 気温/水温の完全一致が0件**。

→ **全体平均比の偏差スコア(0〜100、50=全体平均)** に変更（`GetAnalysisRateController`）:
- `baseline = 全体の釣果ログ / 全体のログ`
- 各条件: `rel = 100 * smoothed^G / (smoothed^G + baseline^G)`（baselineと同じ=50、低いほど0、高いほど100に漸近）
- `smoothed = (caught + K*baseline) / (total + K)`（少サンプルは baseline へ寄せるベイズ平滑化）
- 9条件の rel を平均 → `final = clamp(50 + (avg-50)*FINAL_GAIN, 0, 100)`

**チューニング定数（backend 単独で調整可・アプリ再リリース不要）**:
- `SMOOTHING_K = 8`（小さいほど各条件がデータを主張＝差は出るがノイズ増）
- `CONTRAST_G = 2.0`（写像の急勾配さ。平均から離れた条件を強く振る）
- `FINAL_GAIN = 1.4`（平均化による50回帰の引き戻し）

本番9条件で検証: 条件別 rel が 11〜65 に分散、合成 ≈38。「全部50前後」を解消し採用確定。
native は **0〜100/50=平均** の意味にのみ依存（バンド 68/56/44/32、/100表記、グラフ平均ライン）。**アルゴリズム変更に native 改修は不要**＝定数調整はデプロイのみで既存ユーザーにも反映。

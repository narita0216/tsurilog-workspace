...
5. **ボトムシート型 Modal に TextInput を置くなら `KeyboardAvoidingView` 必須**。
   `justifyContent:flex-end` の Modal は autoFocus でキーボードが開くと保存ボタンが
   キーボードに隠れ、ユーザーもタップできない(QA でも「保存」が押せず保存されなかった)。
   `KeyboardAvoidingView behavior="padding"` で持ち上げる。
   (お気に入り追加時のラベル設定モーダルで実際に踏んだ。2026-06-19 追加要件)

### 追加要件(2026-06-19・コミット `a98519c`)

- お気に入り追加時に**ラベル設定モーダル**を挟む(その場で命名・近隣マスタ名を初期値)。
- ホームのお気に入りカードに**削除**(ゴミ箱→確認アラート→`deletePin`)。
- スクショ `qa-artifacts/20260619-034441/`(01〜11、ラベルモーダル/ホーム削除まで網羅)。

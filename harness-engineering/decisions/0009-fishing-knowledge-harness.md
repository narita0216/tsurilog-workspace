...
## ローカル感の強化(自社DB非依存・2026-06-03)

リリース直後でユーザ40人・釣果データがほぼ無く、自社DBによるローカル実績は当面機能しない。
かつ無料配布は API コストで赤字。そこで**課金前提のまま、無料・確定の外部/同梱データのみで location-specific 化**:
- **地名の確定**(`NearestSpotService`): 同梱500地点から最寄りの県・市・ポイント名を渡し、座標からの推定を排除。
- **海底地形**(`DepthService::describeContour`): 中心+東西南北の水深からかけあがりの向き・レンジを提示(OpenTopoData・無料・キャッシュ)。
- 中身の無い自社「釣行回数」は除去。
- 効果: 「淀川河口付近・西方向へ深くなるかけあがり6→11m・水深5〜7m帯が狙い目」等、地点の地形に最適化された助言に。
- **限界(正直)**: これは公開地形/位置データ由来で“独占”ではない。真の課金 moat は依然「自社の釣果ログ蓄積」。本施策は floor を上げ差別化を稼ぐ繋ぎで、moat は利用が増えるほど効く(ネットワーク効果)。

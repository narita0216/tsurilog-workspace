...
## Consequences
- **得るもの:** ハーネス更新・学び(リポに書いたもの)・テレメトリが自動で GitHub に蓄積。複数マシン間で共有。
- **失うもの:** コミット履歴のノイズ増(autosave コミット多数)。メッセージは機械的。
- **新たに発生する作業:** ノイズが気になれば effectiveness JSONL を gitignore する判断(ただし airtrunk 同様、共有のため現状は追跡)。設定変更後は **Claude Code の再起動**でフック有効化。

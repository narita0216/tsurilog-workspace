...
- **2026-05-29:** Open-Meteo の品質/料金を確認 — Standard **$29/月・100万回・Marine+Weather込み・JMAモデル**で日本高品質(東京で天気/風/気温を実測OK)。tide736.net は736港・無料・実用精度(個人運営=可用性は長期キャッシュで吸収)。→ **ADR-0007 で WWO 全廃を決定**: 天気/海象=Open-Meteo、潮=tide736.net、地形=海しる。**既存の env data 取得・分析ごと移行**する方針。次: backend 移行の実装計画(GetEnvData 書換 / EnvCache TTL / 生値保持 / 海しる地形 / 分析テスト)。

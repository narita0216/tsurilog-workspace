...
> ## ⚠️ 戦略ピボット（2026-06・最重要・最新方針）
> 事業計画をピボットしました。**旧前提（記録フライホイール／有料AIアドバイザー）で動かないこと。**
> - **記録依存をやめる**（記録≒2人でフライホイール死亡）。効いている行動＝**「釣れる度のチェック」**に寄せて monetize。
> - **釣れる度を一般論ベース(config駆動)で再設計**。**無料＝種別非依存の一般スコア（磁石・劣化させない）／有料＝種別ごとスコア（¥300/月・既存tsurilog_lite流用・体感で違うこと。1.1.3）**。
> - **有料AIアドバイザー（ADR-0006/0010・釣りログ Plus ¥500）は一旦廃止/休止**（コードは削除せず非表示。長期R&Dへ格下げ）。既存課金者は丁寧に移行。
> - 黒字化＝**固定費 right-size ＋ 軽い¥300サブ検証**。durable な堀は**流通(動画エンジン)**。
> - 設計の鉄則：記録と基本の釣れる度は永久無料／売るのは深さ・便利さ／見せ方は robust(質的・方向性、精密分解は出さない)／種別は体感で違うこと。
> - **正本**: 判断＝`harness-engineering/decisions/0011-pivot-to-condition-score-monetization.md`、実行計画＝`harness-engineering/initiatives/2026-06-pivot-condition-score-monetization.md`。実装前にここを読む。
...
### 8.0 動作検証はローカルで（AI=ローカル / dev サーバ=人間。最重要・厳守）
> **検証場所の鉄則:** **AI の動作検証は必ず「ローカル」**で行う。**dev / staging / 共有サーバは人間（オーナー）が動作確認する場所であり、AI が「dev はまだ古い」等を理由に検証を省略・先送りするのは禁止。** native の UI 検証も **ローカルで立てた API（Docker の `tsurilog_api`）に向けて**実行する。「dev API が旧レスポンスだから native-qa できない」は誤り＝**ローカル API は自分の変更が入っているのだから、そこに向けて検証する**。
- backend の動作確認は **Docker Compose**(`api`/`queue`/`scheduler`/`db`)で行う。本番 / 共有環境への直接接続で確認しない。
- native は **dev client + `npx expo start --dev-client`**(実機/シミュレータ)。**API 向き先は `EXPO_PUBLIC_API_DOMAIN` でローカル backend を指す**（dev サーバを指さない）。UI を変えたら **`/native-qa`**(シミュレータで動線を通しスクショ撮影 → PR 添付。ADR-0008)で確認する。**本当に実行不能なときだけ**、その技術的理由を具体的に明示する（「dev が古い」は理由にならない）。
- 経緯と教訓 → `findings/2026-06-25-ai-verifies-locally-not-dev.md`（native-qa を「dev が旧レスポンス」を理由に省略しかけた規律違反）。
...
- **【義務】UI変更チェックリスト**(完了report/PR の前に必ず実施。経緯 → `findings/2026-06-28-ui-check-neglect-and-mandate.md`):
  ...
  7. **画像/PDF/SNS共有など「生成物」を伴うUIは、画面の見た目でなく生成された実ファイルを開いて検証**する(ViewShotは表示要素を直接撮らず、画面外の固定サイズ・影なしのキャプチャ専用要素を撮る)。
